...
public class SimpleMatch {
	...
	*/
	int set_number;
	/**
	* @return The color (red/blue) of the winning alliance. Will contain an empty string in the event of no winner,
	* or a tie.
	*/
	...
	*/
	String event_key;
	/**
	* @return TBA event key with the format <code>yyyy[EVENT_CODE]_[COMP_LEVEL]m[MATCH_NUMBER]</code>, where
	* <code>yyyy</code> is the year, and <code>EVENT_CODE</code> is the event code of the event,
	* <code>COMP_LEVEL</code> is (qm, ef, qf, sf, f), and <code>MATCH_NUMBER</code> is the match number in the
	* competition level. A set number may append the competition level if more than one match in required per set.
	*/
	String key;
	/**
	* @return UNIX timestamp (seconds since 1-Jan-1970 00:00:00) of the scheduled match time, as taken from the
	* published schedule.
	*/
	long time;
	/**
